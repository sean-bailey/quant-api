...
# Usage: .\quantize-all.ps1 -ModelFolder "path\to\model\folder" [-BuildDir "path\to\build"] [-IMatrix "path\to\imatrix"]
...
# Clone or update llama.cpp, then ensure binary is built
if (-not $SkipBuild) {
    ...
    if ($ForceRebuild) {
        ...
        if (Test-Path $BuildPath) {
            Write-ColoredOutput "Force rebuild: deleting build directory..." "Yellow"
...
if (!$BaseModel) {
    Write-ColoredOutput "Error: No GGUF files found in $ModelFolder" "Red"
    exit 1
}
...
# Create output folder for quantized models
$OutputFolder = Join-Path $ModelFolder "quantized"
if (!(Test-Path $OutputFolder)) {
    New-Item -ItemType Directory -Path $OutputFolder | Out-Null
    Write-ColoredOutput "Created output folder: $OutputFolder" "Green"
}
