...
# Clean version - outputs are placed next to the base model rather than a subfolder.
# Automatically clones llama.cpp if not present, or pulls latest if it is.
# Usage: ./quantize-all-clean.sh --model-folder "path/to/model/folder" [options]
...
find_base_model() {
    ...
    while IFS= read -r -d '' gguf_file; do
        ...
        for quant in "${QUANT_TYPES[@]}"; do
            ...
            if [[ "$name" == *"$quant_lower"* ]]; then has_quant_suffix=true; break; fi
        done
        if [[ "$has_quant_suffix" == "false" ]]; then echo "$gguf_file"; return 0; fi
...
for i in "${!QUANT_TYPES[@]}"; do
    ...
    for req_type in "${REQUIRES_IMATRIX[@]}"; do
        if [[ "$quant_type" == "$req_type" ]]; then requires_imatrix=true; break; fi
    done
    ...
    args=()
    if [[ -n "$IMATRIX" ]]; then args+=(--imatrix "$IMATRIX"); fi
